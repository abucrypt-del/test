...
function Hero({ onReserve }: { onReserve: () => void }) {
  ...
  const opacity = useTransform(scrollY, [0, 500], [0.95, 0]);
  ...
  return <section id="top" className="relative min-h-[740px] overflow-hidden bg-[#311d18] text-[#f9f3e9]">
    ...
    <div className="relative mx-auto flex min-h-[740px] max-w-[1280px] items-end px-5 pb-20 pt-36 md:px-10 md:pb-28">
...
function Story() {
  return <section id="story" className="bg-[#eee3d2] py-24 text-[#36231a] md:py-36">
    <div className="mx-auto grid max-w-[1280px] gap-16 px-5 md:grid-cols-[.72fr_1.28fr] md:px-10 lg:gap-28">
      <Reveal><div className="flex items-center gap-3 text-[10px] font-bold uppercase tracking-[.28em] text-[#ad542f]"><span className="h-px w-8 bg-[#ad542f]" /> Our table</div><p className="display mt-10 max-w-[280px] text-4xl leading-[1.05] md:text-5xl">From one family kitchen to your favourite table.</p><div className="mt-10 flex items-center gap-3"><span className="display text-5xl text-[#b85e36]">07</span><span className="max-w-[120px] text-[10px] font-bold uppercase leading-4 tracking-[.14em] text-[#705346]">years of gathering people</span></div></Reveal>
    ...
    <div className="mx-auto max-w-[1280px] px-5 md:px-10">
      ...
      <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {filtered.map((item, index) => <Reveal key={item.id} delay={index * 80}><article data-testid={`card-menu-item-${item.id}`} data-food-area className="group overflow-hidden rounded-[1.15rem] border border-[#ddcbb8] bg-[#eee3d2] transition-transform hover:-translate-y-1">
          <div className="relative aspect-[1.05] overflow-hidden"><img src={asset(item.image)} alt={item.name} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" /><span className="absolute left-3 top-3 rounded-full bg-[#f5eee3]/90 px-3 py-1 text-[9px] font-bold uppercase tracking-[.15em] text-[#ad542f]">{item.note}</span></div>
